#include<traj_builder_kmb172/traj_builder_kmb172.h>
...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    nav_msgs::Odometry current_state = vec_of_states.front();//get the current state
    vec_of_states.clear();//empty the trajectory as it is being cancelled for braking
    ...
    des_state.twist.twist.angular.z = 0;//immediately stop spinning as a hack
    
    //TODO: change above line
    while(speed_current > 0){
	speed_current -= accel_max_*dt_; //Euler one-step integration
	des_state.twist.twist.linear.x = speed_current;
        x_current += speed_current * dt_ * cos(psi_current); //Euler one-step integration
        y_current += speed_current * dt_ * sin(psi_current); //Euler one-step integration        
