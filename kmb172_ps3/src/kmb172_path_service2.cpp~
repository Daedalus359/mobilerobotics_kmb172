...
void get_yaw_and_dist(geometry_msgs::Pose current_pose, geometry_msgs::Pose goal_pose,double &dist, double &heading) {
 ...
 dist = sqrt(pow(abs(current_pose.position.x - goal_pose.position.x),2.0)+pow(abs(current_pose.position.y - goal_pose.position.y),2.0)); //pythagorean theorem
 ...
 if (dist < g_dist_tol) { //too small of a motion, so just set the heading from goal heading
   heading = convertPlanarQuat2Phi(goal_pose.orientation); 
    ...
    heading = convertPlanarQuat2Phi(goal_pose.orientation);//just assume the next commanded heading, since it should point towards the next move
 }
